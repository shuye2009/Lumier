...
chdir($dir);
my $result_dir = "Results_oct032018_control_normalization";
mkdir $result_dir unless (-e $result_dir and -d $result_dir);

my %prey_controls = ("Control1" => 1,"Control2" => 1,"Control3" => 1,"Control4" => 1); 
my %bait_controls = ("Flag" => 1, "Flag-GFP" => 1, "H2O" => 1, "H2O-GFP" => 1,
				"TRIM21" => 1, "ILF2" => 1, "ILF3" => 1);  # Blank is not cosidered as it is missing in same plate
...
my %treat_preyControl_mean = (); # store mean of prey controls for each treat, which will be used to scale each 96-well plate
my %treat_prey_baitControl_mean = (); # store mean of bait controls for each treat and each prey, which will be used to scale each 96-well plate
...
my $control_file = $result_dir."/control_of_all_96well_plates.tab";
open(M, ">$control_file") or die $!;

## get means for prey controls and bait controls
print M "Median\tMean\tSTDEV\tMIN\tMAX\tContol1\tControl2\tControl3\tControl4\tFlag\tH2O\tILF2\tILF3\tTRIM21\n";
foreach my $treat (@treatments){
	...
	foreach my $prey (sort @preys){
		...
		while(my ($key, $href) = each %p384_p96_control_lum){
			...
			my $min = $stat->min();
			my $max = $stat->max();
			my $mode = $stat->mode();  ## very sparse data
		...
		while(my ($key, $ref) = each %bait_control_lum){
			...
			$stat->add_data(@ar);
			my $mean = $stat->mean();
			$treat_prey_baitControl_mean{$treat}{$prey}{$key} = $mean;
			#print M "$treat\t$prey\t$key\t$mean\n";
		}
		
	}
	while(my ($key, $ref) = each %prey_control_lum){
		my @ar = @$ref;
		my $stat = Statistics::Descriptive::Full->new();
		$stat->add_data(@ar);
		my $mean = $stat->mean();
		$treat_preyControl_mean{$treat}{$key} = $mean;
...
foreach my $treat (@treatments){
	...
	foreach my $prey (sort @preys){
		...
		while(my ($p384_p96, $lum_href) = each %p384_wellid_p96_lum){
			my $pcontrol_href = $p384_preyControl_p96_lum{$p384_p96};
			my $bcontrol_href = $p384_baitControl_p96_lum{$p384_p96};
			my %pcontrol_hash = %$pcontrol_href;
			my %bcontrol_hash = %$bcontrol_href;
			my @p_weights = ();
			my @b_weights = ();
			
			while(my ($c, $lum) = each %pcontrol_hash){
				my $pmean = $treat_preyControl_mean{$treat}{$c};
				my $w = $lum/$pmean;
				push(@p_weights, $w);
			}
			
			while(my ($c, $lum) = each %bcontrol_hash){
				my $bmean = $treat_prey_baitControl_mean{$treat}{$prey}{$c};
				my $w = $lum/$bmean;
				push(@b_weights, $w);
			}
			
			my $pstat = Statistics::Descriptive::Full->new();
			$pstat->add_data(@p_weights);
			my $pscalar = $pstat->mean();
			my $bstat = Statistics::Descriptive::Full->new();
			$bstat->add_data(@b_weights);
			my $bscalar = $bstat->mean();
			
			my %lum_hash = %$lum_href;
			while (my ($wellID, $lum) = each %lum_hash){

				my $normalized_lum = 0;
				if($pscalar ne 0 and $bscalar ne 0){
					$normalized_lum = $lum/$bscalar;
					#$normalized_lum /= $pscalar; 
				}
