...
for(treatment in c("RN_LYSIS", "SNDREAD")){
  RN_LYSIS <- log10(read.table(file.path(wdir,paste(treatment,"bait_prey_matrix_cluster.tab", sep="_"))))
  NT <- log10(read.table(file.path(wdir,"NT_bait_prey_matrix_cluster.tab")))
  head(RN_LYSIS)
  head(NT)
  ...
  #DIFF[is.na(DIFF)] <- 0
  pdf(paste(treatment, "_NT_log_difference.pdf", sep=""), width=12, height=12)
  heatmap.2(as.matrix(DIFF),trace="none")
  dev.off()
