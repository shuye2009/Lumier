...
wdir <- "C:/RSYNC/LUMIER_C2H2/Archive_oct032018/Archive1_3/Results_oct032018_same_median_normalization"
...
for(treatment in c("RN", "SNDREAD")){
  ...
  DEseq2_table <- read.table(file.path(wdir, "DEseq2", paste("results_of_differential_analysis",treatment, "vs_NT.tab", sep="_")),header=T)  
  RN_LYSIS <- read.table(file.path(wdir,paste(treatment,"replicates_only_median_normalized_lum.tab", sep="_")), header=T)
  NT <- read.table(file.path(wdir,"NT_replicates_only_median_normalized_lum.tab"), header=T)
  head(RN_LYSIS)
  head(NT)
  
  dummy <- c("Blank", "Control1", "Control2", "Control3", "Control4", "Control5", "Control6", "Flag", "Flag-GFP", "H2O", "H2O-GFP")
  
  NT_network <- NT[!is.element(NT[,3], dummy),]
  RN_LYSIS_network <- RN_LYSIS[!is.element(RN_LYSIS[,3], dummy),]
  
  NT_network[,4] <- log10(NT_network[,4])
  RN_LYSIS_network[,4] <- log10(RN_LYSIS_network[,4])
  ...
  colnames(UNION_network) <- c("Prey", "Bait", paste("NT>",sprintf("%.3f",nt_cutoff),sep=""),  paste(treatment,">",sprintf("%.3f",RN_LYSIS_cutoff), sep=""), "Note")
  UNION_network <- merge(UNION_network, DEseq2_table, by.x=c("Bait", "Prey"), by.y=c("baits", "preys"), all=T)
  write.table(UNION_network, paste("NT",treatment,"joint_thresholded_network_replicates.tab",sep="_"), row.names=F, sep="\t")
